...
export default function StudentReportTable({ reports, tasks = [], reviewed, onReviewedChange, basePath = "/mentor" }: { reports: Report[]; tasks?: StudentTask[]; reviewed: "belum" | "sudah" | "belum_dikerjakan"; onReviewedChange: (r: "belum" | "sudah" | "belum_dikerjakan") => void; basePath?: string }) {
        ...
        {list.map((item, i) => (
          ...
          >
            ...
            {item.type === "report" ? (
              ...
              <span className={`px-2 py-1 rounded-(--radius-md) text-xs ${item.status === "submitted" ? "bg-yellow-100 text-yellow-800" :
            ...
            {item.type === "report" ? (
              <Link href={`${basePath}/siswa/${params?.id ?? "STD-001"}/laporan/${item.id}`}>
                <Button variant="destructive" size="sm" className="rounded-full px-4">Lihat Detail</Button>
              </Link>
            ) : (
              <Link href={`${basePath}/tugas/${item.id}/monitoring`}>
